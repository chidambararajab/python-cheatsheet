...
- ❌ Uses fields='__all__' in User serializer → REJECT
...
1. fields='__all__' exposes sensitive data
...
19. **"What's wrong with fields='__all__' in UserSerializer?"**
...
2. **fields='__all__' on User model** - Exposes sensitive data
...
- ❌ Uses fields='__all__' on User model without concern
...
### **IF YOU FAIL:**
*"Candidate has years of experience but fundamental gaps. Would create security vulnerabilities (plain text passwords, data leaks). Cannot optimize queries (N+1). Would struggle in production incidents. Not ready for backend role. **REJECT**."*

### **IF BORDERLINE:**
*"Solid fundamentals but some gaps in security/performance. May need significant mentoring. Consider for junior-mid level with supervision. **DISCUSS WITH TEAM**."*

### **IF STRONG:**
*"Excellent DRF knowledge. Security-conscious. Understands performance optimization. Can debug production issues. Explains concepts clearly. Ready for senior backend role. **RECOMMEND HIRE**."*
...
2. **Uses fields='__all__' by default** - Lazy, insecure
...
- ✅ Never use fields='__all__' without thinking
